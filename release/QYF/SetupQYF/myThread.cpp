...
void myThread::run() {
    ...
    // *****需要修改部分****
    // 启动注射器进程
    BOOL bSuccess = CreateProcessA("D:\\VSProjects\\qyf\\syringe\\Debug\\syringe.exe", filePath, NULL, NULL, FALSE,
        CREATE_NEW_CONSOLE, NULL, NULL, &startupInfo, &processInformation);
    sprintf(temp, "%d", processInformation.dwProcessId);
    emit
        newProcessID(QString(QLatin1String(temp)));
    sprintf(temp, "%s", priorityStr[GetProcessPriority(processInformation.hProcess)]);
    emit
        newProcessPriority(QString(QLatin1String(temp)));
    ...
    if (EnumProcessModules(processInformation.hProcess, hMod, sizeof(hMod), &cbNeeded)) {
        ...
        emit
            newProcessModules(QString(QLatin1String(temp)));
    }
    msleep(1500);
    while (running) {
        if (WaitForSingleObject(hSemaphore, 10) == WAIT_OBJECT_0) {
            memcpy(&recvInfo, lpBase, sizeof(info));
            if (lastSt.wMilliseconds == recvInfo.st.wMilliseconds && lastSt.wSecond == recvInfo.st.wSecond &&
...
void myThread::checkFunc() {
    ...
    case HEAPFREE: {
        ...
        if (heapSet.find(temp) == heapSet.end()) {
            emit newInfo(QString(QLatin1String(
                "warning: The heap repeatedly releases or releases a non-existent heap!\n")), 2);
        }
        else {
            heapSet.erase(temp);
        }
    ...
    case REGOPENKEYEX: {
        if (strstr(recvInfo.argValue[1], "SOFTWARE\Microsoft\Windows\CurrentVersion\Run")) {
            emit newInfo(
                QString(QLatin1String("warning: The program is modifying the startup key in the registry\n")),
                2);
        }
        break;
    }
