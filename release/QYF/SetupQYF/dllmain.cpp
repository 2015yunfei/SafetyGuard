...
struct info {
    int type, argNum;
    SYSTEMTIME st;
    char argName[MAXARGNUM][MAXARGNAMELEN] = { 0 };
    char argValue[MAXARGNUM][MAXARGVALUELEN] = { 0 };
};
...
// 定义需要hook的函数
static int (WINAPI* OldMessageBoxA)(_In_opt_ HWND hWnd, _In_opt_ LPCSTR lpText, _In_opt_ LPCSTR lpCaption, _In_
...
static int (WINAPI* OldMessageBoxW)(_In_opt_ HWND hWnd, _In_opt_ LPCWSTR lpText, _In_opt_ LPCWSTR lpCaption, _In_
    UINT uType) = MessageBoxW;

extern "C" __declspec(dllexport) int WINAPI
NewMessageBoxW(_In_opt_ HWND hWnd, _In_opt_ LPCWSTR lpText, _In_opt_ LPCWSTR lpCaption, _In_ UINT uType) {
    char temp[MAXARGVALUELEN];
...
extern "C" __declspec(dllexport)HANDLE WINAPI NewCreateFile(
    ...
    HANDLE hFile = OldCreateFile(lpFileName, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition,
        dwFlagsAndAttributes, hTemplateFile);
    if (GetFileType(hFile) == FILE_TYPE_DISK) {
        // 检查文件的类型是否为磁盘文件。只有当文件类型为磁盘文件时，才会执行后续操作
        char temp[MAXARGVALUELEN];
...
extern "C" __declspec(dllexport)HANDLE WINAPI
NewHeapCreate(DWORD fIOoptions, SIZE_T dwInitialSize, SIZE_T dwMaximumSize) {
...
extern "C" __declspec(dllexport)LSTATUS WINAPI NewRegCreateKeyEx(
    ...
    memcpy(lpBase, &sendInfo, sizeof(info));
    ReleaseSemaphore(hSemaphore, 1, NULL);
    return OldRegCreateKeyEx(hKey, lpSubKey, Reserved, lpClass, dwOptions, samDesired, lpSecurityAttributes, phkResult,
        lpdwDisposition);
}
